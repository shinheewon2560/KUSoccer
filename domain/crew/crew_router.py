...
async def add_leader(crew_id : int , request : crew_schema.UserEmail, request_user_id : int = Depends(get_id_from_doubly_verified_token), db : AsyncSession = Depends(get_DB)):
...
async def delete_leader(crew_id : int , request_user_id : int = Depends(get_id_from_doubly_verified_token), db : AsyncSession = Depends(get_DB)):
    await crew_crud.delete_leader_in_db(crew_id,request_user_id, db)

@router.put("/{crew_id}/Leader/Profile", status_code= 200)
async def modify_crew_profile(crew_id : int, request : crew_schema.ModifyCrewRequest, request_user_id : int = Depends(get_id_from_doubly_verified_token), db : AsyncSession = Depends(get_DB)):
...
async def accept_user(response : Response, crew_id : int, request : crew_schema.CrewAcceptRequest,request_user_id : int = Depends(get_id_from_doubly_verified_token), db : AsyncSession = Depends(get_DB)):
    _result = await crew_crud.accept_user_in_db(crew_id,request, request_user_id, db)
    response.status_code = status.HTTP_201_CREATED
    return _result

@router.post("/{crew_id}/Leader/member")
async def add_member(response : Response, crew_id : int, request : crew_schema.UserEmail, request_user_id : int = Depends(get_id_from_doubly_verified_token), db : AsyncSession = Depends(get_DB)):
    _result = await crew_crud.add_member_in_db(crew_id,request.e_mail,request_user_id, db)
    response.status_code = status.HTTP_201_CREATED
    return _result

@router.delete("/{crew_id}/Leader/member")
async def delete_member_by_leader(response : Response, crew_id : int, request : crew_schema.UserEmail , request_user_id : int = Depends(get_id_from_doubly_verified_token), db : AsyncSession = Depends(get_DB)):
    await crew_crud.delete_member_by_leader_in_db(crew_id, request.e_mail, request_user_id, db)
    response.status_code = status.HTTP_204_NO_CONTENT

@router.delete("/{crew_id}/Leader/")
async def delete_crew(response : Response, crew_id : int, request_user_id : int = Depends(get_id_from_doubly_verified_token), db : AsyncSession = Depends(get_DB)):
