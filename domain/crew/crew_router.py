...
def delete_member_by_self(response : Response, crew_id : int, request_user_id : int = Depends(check_token_and_return_id), db : Session = Depends(get_DB)):
...
def add_member(response : Response, crew_id : int, request : crew_schema.UserEmail, request_user_id : int = Depends(check_token_and_return_id), db : Session = Depends(get_DB)):
    _result = crew_crud.add_member_in_db(crew_id,request.e_mail,request_user_id, db)
    response.status_code = status.HTTP_200_OK
    return _result

@router.delete("/{crew_id}/Member")
def delete_member_by_leader(response : Response, crew_id : int, request : crew_schema.UserEmail , request_user_id : int = Depends(check_token_and_return_id), db : Session = Depends(get_DB)):
    _result = crew_crud.delete_member_by_leader_in_db(crew_id, request.e_mail, request_user_id, db)
    response.status_code = status.HTTP_200_OK
    return _result

@router.delete("/{crew_id}")
def delete_crew(response : Response, crew_id : int, request_user_id : int = Depends(check_token_and_return_id), db : Session = Depends(get_DB)):
