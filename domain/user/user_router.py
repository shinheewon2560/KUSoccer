...
#depondencies는 문서화를 위한 파라미터
@router.get("/Profile", dependencies=[Depends(user_crud.get_id_from_token)])
...
#2차 검증이라고 보면 됨
@router.post("/Auth", dependencies=[Depends(user_crud.decode_token)])
...
@router.get("/Profile/Me", dependencies=[Depends(user_crud.get_id_from_token)])
...
@router.put("/Profile/Me", dependencies=[Depends(user_crud.get_id_from_doubly_verified_token)])
...
@router.delete("/Me", dependencies=[Depends(user_crud.get_id_from_doubly_verified_token)])
